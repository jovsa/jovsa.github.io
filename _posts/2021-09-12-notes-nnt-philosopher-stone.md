...
categories: notes
modified_date: "Sept 14, 2021"
author:
- Jovan Sardinha
---

> summary from Book V, Chapter 19
...
*A toy example*: The number of cars is the `variable`; traffic time is the `function` of the `variable`. The behavior of the function is “not the same thing”. We can see here that the `function` becomes different from the `variable` under nonlinearities.

* (a) The more nonlinear, the more the function divorces itself from the variable. If traffic were linear, then there would be no difference in traffic time between the two following situations: 90,000, then 110,000 cars on the one hand, or 100,000 cars on the other.
* (b) The more volatile the variable, the more the function divorces itself from the variable. Let us consider the average number of cars again. The function (travel time) depends more on the volatility around the average. Things degrade if there is unevenness of distribution. For the same average you prefer to have 100,000 cars for both time periods; 80,000 then 120,000, would be even worse than 90,000 and 110,000.
* (c) If the function is convex (antifragile), then the average of the function is going to be higher than the function of the average of each variable. And the reverse when the function is concave (fragile).
...
* As an example for (c), which is a more complicated version of the bias, assume that the function under question is the squaring function (multiply a number by itself). This is a convex function. Take a conventional die (six sides) and consider a payoff equal to the number it lands on, that is, you get paid a number equivalent to what the die shows—1 if it lands on 1, 2 if it lands on 2, up to 6 if it lands on 6. The square of the expected (average) payoff is then $$(\frac{1+2+3+4+5+6}{6})^2$$, equals $$3.5^2$$, here 12.25. So the function of the average equals 12.25.
* But the average of the function is as follows. Take the square of every payoff, $$\frac{1^2+2^2+3^2+4^2+5^2+6^2}{6}$$, that is, the average square payoff, and you can see that the average of the function equals 15.17.
* So, since squaring is a convex function, the average of the square payoff is higher than the square of the average payoff. The difference here between 15.17 and 12.25 is what I call the hidden benefit of antifragility—here, a 19 percent “edge.”

**Concave Example**:

* The square root of the expected (average) payoff is then $$\frac{\sqrt{1+2+3+4+5+6}}{6}$$, equals $$\sqrt{3.5}$$, here 1.87. The function of the average equals 1.87.
* But the average of the function is as follows. Take the square root of every payoff, $$\frac{\sqrt{1}+\sqrt{2}+\sqrt{3}+\sqrt{4}+\sqrt{5}+\sqrt{6}}{6}$$, that is, the average square root payoff, and you can see that the average of the function equals 1.80.


## Modelling Second Order Effects and Fragility

> summary from the appendix II
